...
Bitmap FreeTypeCharacterBitmapProvider::storeNewGlyph(char c,
                                                      unsigned int pixelSize) {
    ...
    const auto &built = cache[pixelSize].emplace(c, Bitmap {
        face->glyph->bitmap.rows, face->glyph->bitmap.width, GL_RED,
