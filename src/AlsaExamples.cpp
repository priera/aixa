...
int write_and_poll_loop(snd_pcm_t *handle,
    ...
    ufds = static_cast<pollfd *>(malloc(sizeof(struct pollfd) * count));
    if (ufds == NULL) {
    ...
    while (1) {
        ...
        switch (c) {
            ...
            case 'm':
                ...
                break;
            case 'o': {
                int iFormat = static_cast<int>(SND_PCM_FORMAT_S8);
                int iFormatmax = static_cast<int>(SND_PCM_FORMAT_LAST);

                for (iFormat; iFormat < iFormatmax; iFormat++) {
                    const char *format_name = snd_pcm_format_name(static_cast<_snd_pcm_format>(iFormat));
                    if (format_name)
                        if (!strcasecmp(format_name, optarg))
                            break;
                }

                if (iFormat == SND_PCM_FORMAT_LAST)
                    iFormat = SND_PCM_FORMAT_S16;

                format = static_cast<_snd_pcm_format>(iFormat);
